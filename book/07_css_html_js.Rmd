...
Often, you'll need to find out how to refer to a specific element or group of elements that were created by a `r pkg("shiny")` ui function. For example, it isn't obvious how you'd refer to the sidebar tab for `r dt("x_tab")` in the following code:
...
Now you know that the dragon icon is made by an italics tag (`r dt("<i>", "kw")`) that's inside an anchor (`r dt("<a>", "kw")`) with the `r dt("id", "er")` `r dt("#shiny-tab-x_tab", "pp")`. Therefore, you can change the colour of this icon using the following CSS:
...
Add the following HTML before the image in `x_tab` using ``r func("HTML")`:

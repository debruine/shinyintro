...
I prefer ggplots, so let's make the plot with `geom_histogram` instead of `hist` (which is a great function for really quick plots). Since we need several functions from the `ggplot2` `r glossary("package")`, we'll need to load that package at the top of the script, just under where the `shiny` package is loaded:
...
Add the following code as the first argument to `sidebarPanel()`, which just takes a list of different widgets. `radioButtons` is the widget we're using. We'll set four `r glossary("argument", "arguments")`:
...
```{r first-demo-app, eval = TRUE, echo=FALSE, fig.cap="First Demo App. You can also access this app with `shinyintro::app(\"first_demo\")`."}
...
Inputs are ways for the users of your app to communicate with the app, like drop-down menus or checkboxes. We'll go into the different types of inputs in Capert\ \@ref(inputs).  Below we'll turn the list of favourite things into a group of checkboxes
...
Reactive functions are functions that only run when certain types of inputs change. Inside the server function, the `r glossary("object")` `input` is a named `r glossary("list")` of the values of all of the inputs. For example, if you want to know which items in the select input named "fav_things" were selected, you would use `input$fav_things`.

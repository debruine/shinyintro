...
You can also change the default `r arg("skin")` colour. Possible skin colours are: red, yellow (looks orange to me), green, blue, purple, and black.
...
Since each tab is usually a quite complex list of elements, I like to define each tab outside `r func("dashboardPage")` and then just put a list of the tab objects inside `r func("tabItems")`. This way, it's easy to move the whole tab definition to an external file if it gets too complex (see Chapter \ \@ref(structure)).
...
`r pkg("Shinydashboard")` uses a grid system that is 12 units across. The default width of boxes is 6, and info and value boxes are 4.
...
* Experiment with different values for the `r arg("titleWidth")` argument. Try "50%" or "200px" and see how the title behaves when you change the width of the browser window.

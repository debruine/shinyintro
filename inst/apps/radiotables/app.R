...
server <- function(input, output, session) {
    ...
    output$pet_plot <- renderPlot({
        debug_msg("pet_plot")
        data <-
            get_radio_table_inputs("pet_table", input, "long") %>%
        ...
        ggplot(data, aes(
            ...
            scale_y_continuous(name = "Pet Preference",
                               breaks = 1:9,
                               limits = c(1, 9))
    })
    
    ### pet_summary ----
    output$pet_summary <- renderPlot({
        debug_msg("pet_summary")
        if (nrow(v$pet_summary_data) == 0)
            return()
    ...
    ## food ----
    ### food_submit ----
    observeEvent(input$food_submit, {
        debug_msg("food_submit")
        # save the data and update summary data
    ...
    ### food_summary ----
    output$food_summary <- renderPlot({
        debug_msg("food_summary")
        if (nrow(v$food_summary_data) == 0)
            return()
